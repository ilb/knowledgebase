...
privapi.openLibreOffice = function (file_) {
    if (!window.privilegedAPI || !privilegedAPI.availiable()) {
        alert('Not available!');
        return false;
    }
    var file = file_ || '';
    ...
    privilegedAPI.exec(
        cmd,
        function (res) {
            console.log('EXEC_SUCCESS_CALLBACK:', res);
        },
        function (res) {
            console.log('EXEC_ERROR_CALLBACK:', res);
        }
    );
};
...
var editDoc = function () {
    ...
    if (headURL) {
        privapi.openLibreOffice(headURL);
    }
};
...
var openDocHistory = function () {
    var headURL = getHeadURL();
    if (headURL) {
        window.open(headURL.replace('/repos/', '/viewvc/').replace('svn.net.ilb', 'svn.ilb'), '_blank');
    }
};
...
(function () {
    ...
    var appendSettings = function () {
        ...
        createButton.setAttribute('onclick', 'createDocument()');
...
(function () {
    ...
    var parseContent = function (table, parent) {
        ...
        menuElems = menuElems.filter(function (el) {
            return el && (!el.parentLink || !parent);
        }); // remove parent directory link from sub menu
        if (!menuElems.length) {
            menuElems.push({name: 'Пустой каталог'});
        }
...
function createForm() {
    var span = document.querySelector(".fon .window > span"),
